      ...
      <main className="py-20">
        <section className="mx-auto max-w-7xl px-6">
          ...
          <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-6">
            {STATS.map(({ icon: Icon, k, v }) => (
              <div key={v} className="rounded-2xl border border-border bg-card p-6 flex flex-col items-center text-center hover:border-brand hover:-translate-y-1 transition-all shadow-sm">
        ...
        <section className="mx-auto max-w-7xl px-6 py-20 mt-20 border-t border-border">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {TESTIMONIALS.map((t) => (
              <figure key={t.name} className="relative rounded-2xl border border-border bg-card p-8 hover:border-brand transition shadow-sm">
                <Quote className="absolute top-6 right-6 h-8 w-8 text-brand/20" />
                <blockquote className="text-foreground/80 leading-relaxed relative z-10 text-sm">"{t.body}"</blockquote>
